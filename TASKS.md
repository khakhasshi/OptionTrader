...
- 🔄 Phase 3：候选交易与半自动闭环。可信 ThetaData 期权证明、IBKR 动态账户事实账本与重启对账已实现；真实 Broker 提交、Longbridge 自动恢复、outbox/密钥轮换及 paper 现场认证仍待完成。
...
| P3-G Longbridge/IBKR adapter | 🔄 | Longbridge 4.3.3 Rust SDK 已接账户/持仓/全部活动订单/当日成交、BUY-first 拆腿和未知订单闭锁。IBKR sidecar 已接四类快照、OPT/BAG、MKT/LMT/Adaptive、跨进程重启 orderRef+shape 幂等和只读恢复；全账户订单按配置 account 双重过滤。真实 adapter 提交选择、Longbridge 自动恢复和 paper 现场认证待补。 |
