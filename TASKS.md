...
| P3-C Rust 两阶段硬风控 | 🔄 | ThetaData exact-contract quote/size/first-order Greeks bridge 与内容寻址批次已完成，Rust registry 在 Stage/Confirm 逐字段验证；Standard 无二阶 Gamma，改用同步 ThetaData Delta/IV/underlying/time 推导并拒绝混时数据。Q3 参数批准与完整 RTH option soak 待补。 |
| P3-D 状态机与 paper adapter | 🔄 | 已覆盖确认、幂等、三类订单语义、自适应定价、父子订单、残仓、提交/部分成交/拒单/撤单/断线；PostgreSQL workflow 可原子重建，已提交/已 claim 非终态强制提升 ReconcilePending 且版本递增。真实 Broker 自动对账仍待接线。 |
| P3-E PostgreSQL 审计与恢复保护 | 🔄 | 计划/风险/订单/确认意图/状态事件事务化；Fernet capability 一次性 claim；state_version/行锁阻止回退；启动时自动读取 plan/order/capability 并调用 RestoreWorkflow，恢复响应再次持久化。outbox、密钥轮换及 reconciliation worker 待补。 |
...
| P3-G Longbridge/IBKR adapter | 🔄 | Longbridge 4.3.3 Rust SDK 已接账户/持仓/全部活动订单/当日成交、BUY-first 拆腿和未知订单闭锁。IBKR loopback gRPC sidecar 已接 TWS/Gateway 四类快照 end-callback、OPT/BAG、MKT/LMT/Adaptive、幂等提交/撤单与未知活动订单闭锁。Rust 主 workflow 的真实 adapter 选择和 paper 现场认证待补，live 提交保持禁用。 |
| P3-H Gate/E2E | 🔄 | 已入库部分成交→断线→重启残仓保持、拒单重启不重发、IBKR 真 gRPC snapshot/reconcile、Theta 批次防伪测试；仍需完整 RTH option soak、真实 paper 账户故障演练、保护性退出和 Q3 参数批准后签收。 |
