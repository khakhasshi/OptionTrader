...
- ✅ Phase 4 自动编排：盘后确定性聚合、盘中异步触发、outbox/DLQ、PostgreSQL 多 worker 配额与 single-flight
...
- 🔄 Phase 4 长期评测：扩充历史交易日 corpus、记录模型/提示词版本漂移与成本趋势。
...
### Phase 4 LLM 辅助与复盘（代码完成；长期评测持续）

| 任务 | 状态 | 说明 |
|---|---|---|
| P4-A 契约与安全边界 | ✅ | `llm_review_request.json` / `llm_review.json` + Pydantic 严格模型；输入白名单、大小/深度/有限数校验、secret-like 字段与中英文注入拦截、来源 ID 校验。 |
| P4-B Provider 与可靠性 | ✅ | OpenAI-compatible JSON mode；超时/429/5xx/非法 JSON/Schema/阶段约束受限重试；完整 prompt 成本按最坏尝试数预留；失败路径累计实际 token 遥测；usage 六层硬上限；PostgreSQL 原子每日配额、request-id single-flight、租约和崩溃惰性恢复。进程内保留 TTL 内容 cache 与并发信号量。任何失败均惰性 Review Only。 |
...
| P4-F 自动编排 | ✅ | XNYS 收盘/早收盘调度；从 PostgreSQL 聚合信号、候选、订单、成交、风控、事件与未交易原因；缺证据或同 session Broker 对账未完成保持 `WAITING_INERT`，不调用 Provider。盘中白名单状态事件经独立 cursor、指纹去重、去抖、限频和批量合并进入 outbox；消费支持租约、重试、dead letter。固定 request id 重启不重复调用 Provider。默认 opt-in 关闭。 |
...
> 2026-07-22 Phase 4 主体 Gate：`make lint` 三端全绿；`make test` 为契约 45 / React 84 / Python 292（+5 环境门控 skip）/ Rust 118（+2 显式 live ignored）/ 强制跨语言 integration 1。PostgreSQL 16 以两条旧 LLM 与两条旧 Daily Review 行完成 `0005→0006→0005→0006`：每行 legacy hash 独立、超限 token 被 DB CHECK 拒绝、回滚恢复原 JSON，45 项真实 persistence 测试全绿。DeepSeek v4 flash live smoke 1 项通过；5 case 对抗评测 structured=1、conflict recall=1、false positive=0、injection block=1、unavailable inert=1，missed/contract/expectation mismatch 均为空。真实密钥只在 Git ignored、mode 0600 的本地 `.env`，未进入提交。

> 2026-07-22 独立评审整改与 Rust 可维护性重构：评审 4 个 P2 已闭合，另将 `source_refs` 纳入注入过滤；同进程并发相同请求只产生一次 Provider 调用，同 request id 不同输入在网络前冲突。`risk_grpc.rs` 从 3442 行降至约 500 行，按 backend、candidate、confirmation、orders、reconciliation、mapping 与 tests 拆分，gRPC 公共接口和 24 项风控行为回归保持不变。`repository.py`、`longbridge.rs`、`broker_registry.rs` 仍列为后续可维护性拆分候选，不在本次纯结构重构范围内。

> 2026-07-22 Phase 4 自动编排 Gate：P4-F1/F2/F3 已完成，自动调用默认关闭。`make lint` 三端全绿；`make test` 为契约 47 / React 85 / Python 311（+8 环境门控 skip）/ Rust 118（+2 显式 live ignored）/ 强制跨语言 integration 1；离线 LLM eval 2 项通过。隔离 PostgreSQL 16 完成 `0007→0006→0007`，回滚后新表 0、重升级后 5、关键约束 3/3；多 worker 原子预算、全局 request-id single-flight、盘中批次唯一消费及既有持久化共 67 项通过。故障注入覆盖缺数据/未收盘/非终态订单/父子腿 Broker identity/跨 session 快照/重复事件/篡改 outbox/死信/租约过期/重启，均不重复调用 Provider。变更文件 Gitleaks 43/43 无泄漏；本轮未调用真实 LLM，未运行任何 Broker mutation。
