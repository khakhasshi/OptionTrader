...
| P3-D 状态机与 paper adapter | 🔄 | 已覆盖确认、幂等、三类订单语义、Rust package/每腿自适应定价、父子订单、残仓、提交/部分成交/拒单/撤单/断线与 ReconcilePending；ExecutionOrder 1.1 投影完整子单数量/成交/状态，三端独立校验残余敞口，API 拒绝回退、子单消失及无终态证明的残仓清除。workflow 自动重建和真实 broker 对账仍未完成。 |
...
| P3-G Longbridge/IBKR adapter | 🔄 | Longbridge 4.3.3 Rust SDK 已接账户/持仓/单腿及 BUY-first 受控拆腿、逐腿成交轮询、父子单对账与撤单；SDK 订单 I/O 窄适配层已有部分成交撤单、撤单失败、超时、终态与恢复重复匹配测试。IBKR 已完成 TWS/Gateway 严格端点、nextValidId/account 握手、OPT/BAG 与 MKT/LMT/Adaptive 映射。订单/成交全量流、sidecar gRPC、自动重启恢复和 paper 现场认证待补，live 提交保持禁用。 |
