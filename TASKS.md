...
| P2-B Rust 快照流 + DataHealth | ✅ | `market-core/health.rs` 状态机、replay source、Theta STOCK/TRADE WebSocket + 当日 OHLC REST 回补；tonic :50051 与 axum :8080 同进程。 |
...
| P2 ThetaData 实时流 | ✅ 代码 / ⏳ 现场 | 单 WebSocket 生产者、严格解析、RTH 1m 聚合、盘中/重连 REST 回补、静默 watchdog、动态 gRPC 与 mock 集成测试完成；真实 Standard entitlement、脱敏字段样本和完整 RTH soak 待 Terminal 运行后验收。 |
...
> 2026-07-21 Phase 2 剩余开发：新增四类事件输入契约与 EventContext file importer，覆盖 ET 日期、来源时间、置信度、持仓时效、事件窗口及 PostgreSQL/审计写入；Projector/Strategy/API/React 最终闸门均要求事件上下文 available。Rust 新增 Theta v3 STOCK/TRADE WebSocket、严格 sequence/RTH 校验、分钟聚合、当日 OHLC REST 回补、动态 gRPC、多订阅与静默 watchdog；本地 mock WebSocket 已驱动真实动态 gRPC。现场仍需启动用户的 Theta Terminal 完成 entitlement/字段样本和完整交易日 soak，因此 Phase 3 开发可并行开始，但 paper/live Gate 不得据此签收。
