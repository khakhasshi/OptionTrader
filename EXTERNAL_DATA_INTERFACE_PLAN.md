...
> 系统：QQQ 日内期权波动率交易驾驶舱
> 版本：v0.4
> 当前已具备：ThetaData Options Standard、ThetaData VIX 数据、Longbridge、IBKR、DeepSeek OpenAI-compatible LLM
...
DeepSeek/OpenAI-compatible Provider = 只读解释与复盘，不构成 Market/Execution Truth
...
定位：非权威的解释、SOP 语义审阅、盘后归因和研究假设生成。

当前开发配置使用 DeepSeek OpenAI-compatible `/chat/completions` + JSON mode。Provider 只接收
经过白名单和脱敏的结构化上下文，不接收 Broker 凭证、账户原始数据或 `raw_ref`，也没有工具、
下单、撤单、平仓和风险参数接口。超时、限流、无效输出或预算耗尽只生成惰性的
`Review Only`，不得改变 Market Data Truth、Execution Truth 或 Rust 硬风控结论。
