...
- **D6**：Phase 3 半自动执行——CandidateTradePlan 1.1 使用确定性 Protobuf hash并携带每腿 quote/size/Greeks/chain proof；Rust 在 Stage 与人工确认后各执行一次权威风控，校验市场/事件/账户/策略白名单/开仓窗口/规则。确认令牌不进浏览器/日志，以 Fernet 密文进入 PostgreSQL 共享 capability store，并在确认意图事务内一次性 claim；不确定结果先对账。订单由 Rust 单调 `state_version` 仲裁，PostgreSQL 独立列+行锁+条件更新及 React last-known 锚点共同阻止回退。自适应限价只由 Rust 计算并受计划保护价约束，坏报价不退化；市价新开仓闭锁。REPLAY/SHADOW 不接 Broker，PAPER/MANUAL_CONFIRM 当前只接确定性 PaperBroker，CONTROLLED_AUTO/live 禁用。Longbridge 使用官方 Rust SDK且多腿 fail closed；IBKR TWS/Gateway 使用 BAG，真实订单/成交流、sidecar gRPC 与重启对账仍未完成。风险参数默认未确认、购买力默认 0。详见 `docs/adr/0002-phase3-semi-automated-execution.md` 与 `docs/BROKER_ADAPTERS.md`。
- 后续决策追加于此并同步 ADR。
