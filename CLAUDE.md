...
- **D6**：Phase 3 半自动执行——CandidateTradePlan 使用确定性 Protobuf hash；Rust 在 Stage 与人工确认后各执行一次权威风控，并校验最新市场/事件/账户/规则。确认令牌不落库、不进浏览器，Application API 强制单 worker；PostgreSQL 先记录确认意图，Rust 再提交。订单由 Rust 单调 `state_version` 仲裁，workflow 原地推进，错误路径保留执行事实并进入对账。REPLAY/SHADOW 不接 Broker，PAPER/MANUAL_CONFIRM 当前只接确定性 PaperBroker，CONTROLLED_AUTO/live 禁用。Broker 请求必须携带完整等量多腿语义；Longbridge 原生 adapter 与 IBKR sidecar 统一服从 `broker.proto`，但真实适配和重启对账尚未完成。风险参数默认未确认、购买力默认 0。详见 `docs/adr/0002-phase3-semi-automated-execution.md`。
- 后续决策追加于此并同步 ADR。
