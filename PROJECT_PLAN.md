...
gRPC（Python↔Rust）：StreamMarketSnapshots、GetDataHealth、EvaluateRisk、SubmitApprovedPlan、CancelOrder、ClosePosition、StreamOrderEvents、GetBrokerSnapshot、ReconcileState、ActivateKillSwitch。`SubmitApprovedPlan` 须携带人工确认令牌、确认时间、plan hash、规则版本、过期时间。
...
订单状态机：Proposed →(风控) RiskRejected | AwaitingConfirmation →(确认) Approved →(二次风控) Submitting → Working → PartialFill → Filled；分支 Cancelled / Rejected / Expired。终态：Filled / Cancelled / Rejected / RiskRejected / Expired。
...
Phase 0 已通过 Gate Review（2026-07-20），工程与契约基础技术验收通过，正式进入 Phase 1（历史数据与离线回放，待细化）。详见 `TASKS.md`。
