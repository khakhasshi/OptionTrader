...
from app.regime.engine import _vwap_crossings
...
def test_strong_uptrend_scores_trend() -> None:
    ...
    state = evaluate(
        _bars(closes), inputs=RegimeInputs(volume_vs_20d=1.3, straddle_series=[1.0, 2.0])
    )
...
def test_choppy_session_scores_range() -> None:
    ...
    state = evaluate(
        _bars(closes), inputs=RegimeInputs(volume_vs_20d=0.7, straddle_series=[2.0, 1.0])
    )
...
def test_vwap_crossings_has_no_off_by_one() -> None:
    close = pd.Series([101.0, 99.0, 101.0, 99.0, 101.0])
    vwap = pd.Series([100.0] * 5)
    assert _vwap_crossings(close, vwap) == 4


def test_late_session_data_forces_no_trade() -> None:
    bars = _bars([500.0 + i * 0.5 for i in range(30)])
    bars["occurred_at_utc"] += pd.Timedelta(minutes=30)
    bars["timestamp_et"] += pd.Timedelta(minutes=30)
    state = evaluate(bars, inputs=RegimeInputs(volume_vs_20d=1.3, straddle_series=[1.0, 2.0]))
    assert state.regime == NO_TRADE
    assert "opening_range_complete" in state.unavailable
...
def test_both_scores_high_is_chaos() -> None:
    ...
    state = evaluate(_bars(closes, highs=highs, lows=lows), opening_range_minutes=3)
